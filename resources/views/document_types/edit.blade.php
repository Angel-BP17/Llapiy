...
@section('content')
            ...
            <form action="{{ route('document_types.update', $documentType) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="name" name="name"
                        value="{{ $documentType->name }}" required>
                </div>

                <div class="form-group">
                    <label>Seleccionar Grupos y Subgrupos</label>
                    <div id="groupTree" class="border p-3 rounded" style="max-height: 300px; overflow-y: auto;"></div>
                </div>
    ...
    <script>
        ...
        window.selectedCampoTypes = @json($documentType->campoTypes->pluck('id'));

        window.areas = @json($areas);
        window.preselectedGroups = @json($documentType->groups->pluck('id'));
        window.preselectedSubgroups = @json($documentType->subgroups->pluck('id'));
    </script>
@endsection
