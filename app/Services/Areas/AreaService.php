        ...
        $query = Area::query()
            ...
            ->with([
                'groups' => function ($query) {
                    $query->select('groups.id', 'groups.area_group_type_id', 'groups.descripcion', 'groups.abreviacion')
                        ->withCount(['subgroups', 'users', 'documentTypes']);
                },
                ...
                'groups.subgroups' => function ($query) {
                    $query->select('subgroups.id', 'subgroups.group_id', 'subgroups.descripcion', 'subgroups.abreviacion', 'subgroups.parent_subgroup_id')
                        ->withCount(['subgroups', 'users', 'documentTypes'])
                        ->orderBy('subgroups.descripcion');
                },
            ])
            ->withCount('groups')
            ->orderBy('descripcion');
