...
class HomeService
{
    ...
    public function getDashboardData(): array
    {
        ...
        $isColaborador = $user->hasRole('COLABORADOR_DOCUMENTAL');
        $driver = DB::connection()->getDriverName();

        // 1. CONSULTAS BASE (Para reutilización y limpieza)
        if ($isColaborador) {
            $docBaseQuery = Document::query()
                ->where('group_id', $user->group_id)
                ->where('subgroup_id', $user->subgroup_id);
            $blockBaseQuery = Block::query()
                ->where('group_id', $user->group_id)
                ->where('subgroup_id', $user->subgroup_id);
        } else {
        ...
        $dateExpression = $driver === 'pgsql' ? 'created_at::date' : 'DATE(created_at)';
        $documentosRecientes = (clone $docBaseQuery)
            ->selectRaw("{$dateExpression} as fecha, COUNT(*) as cantidad")
        ...
        $monthlyExpression = match ($driver) {
        ...
        // 6. DATOS PARA EL EMBUDO DE DIGITALIZACIÓN
        $totalBlocks = (clone $blockBaseQuery)->count();
        $totalRegistered = (clone $docBaseQuery)->count();
        $totalStored = (clone $blockBaseQuery)->whereNotNull('box_id')->count();

        $funnelData = [
            ['name' => 'Entrada (Bloques)', 'value' => $totalBlocks + $totalRegistered, 'fill' => '#8884d8'],
            ['name' => 'Procesados (Documentos)', 'value' => $totalRegistered, 'fill' => '#83a6ed'],
            ['name' => 'Finalizados (Almacenados)', 'value' => $totalStored, 'fill' => '#82ca9d'],
        ];

        // 7. MAPA DE ACTIVIDAD (Top Usuarios)
        $activityQuery = \App\Models\ActivityLog::with('user:id,name')
        ...
        $activityStats = $activityQuery->groupBy('user_id')
            ->orderBy('total', 'desc')
            ->take(5)
            ->get()
            ->map(fn($log) => [
